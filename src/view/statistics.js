...
export default class Statistics extends Smart {
  constructor(cards) {
    super();
    this._data = cards;
    this._statisticData = this._data;
  ...
  getTemplate() {
    this._uniqGenre = Object.keys(createGenres(this._statisticData));
    this._uniqGenreQuantitys = Object.values(createGenres(this._statisticData));
    return createStatsTemplate(this._statisticData, this._uniqGenre, this._activeFilter);
  }
  ...
  _formChangehandler(evt) {
    ...
    if (evt.target.tagName === `INPUT`) {
      ...
      switch (this._activeFilter) {
        case `all-time`:
          this._statisticData = this._data.filter((item) => dayjs(item.watchDate));
          break;
        case `today`:
          this._statisticData = this._data.filter((item) => dayjs(item.watchDate).isAfter(dayjs().add(-1, `day`)));
          break;
        case `week`:
          this._statisticData = this._data.filter((item) => dayjs(item.watchDate).isAfter(dayjs().add(-1, `week`)));
          break;
        case `month`:
          this._statisticData = this._data.filter((item) => dayjs(item.watchDate).isAfter(dayjs().add(-1, `month`)));
          break;
        case `year`:
          this._statisticData = this._data.filter((item) => dayjs(item.watchDate).isAfter(dayjs().add(-1, `year`)));
          break;
      }
      this.updateElement(this._statisticData);
    }
  }
